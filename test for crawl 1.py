...
def get_review(url_df, folder_name):
    ...
    PATH = r"C:\Users\msthu\PycharmProjects\pythonProject1\semester5\datascience\chromedriver.exe"  # path to the webdriver file
    ...
    for i in range(0, len(url_df['review_link'])):
        ...
        driver.implicitly_wait(
            2)  # tell the webdriver to wait for 1 second for the page to load to prevent blocked by anti spam software
        ...
        review['movie_id'] = mid
        review.to_csv(
            f'data/{folder_name}/{mid}.csv')  # store them into individual file for each movies, so we can combine or check them later
        driver.quit()
    print("Done processing")
movies_imdb = movies = pd.read_csv('data/moviesurl.csv')
movies_imdb = movies_imdb.drop(movies_imdb.columns[movies_imdb.columns.astype(str).str.contains('Unnamed', case=False)],
                               axis=1)
